...
printf("\n");
for (i = 0; i < nstates; i++) {printf(" %c | ", 'A'+i); /* state */
...
state_flags[i] = '\0';
printf(" %d:[%s]\t--> [%s] (%s)\n",
cur, stnt[cur], state, state_flags);if (i=is_one_nextstate(state_flags))
...
for (i = 0; i < n; i++) {
/* for each pseudo-DFA state */for (j = 0; j < n_sym; j++) { /* for each input symbol */
